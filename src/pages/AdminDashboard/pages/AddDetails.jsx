...
export default function AddDetails() {
  ...
  const [isLoadingOvalupdate, setIsLoadingOvalupdate] = useState(false);
    const getSiteData = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`${import.meta.env.VITE_URL_BACKEND}/siteData/${import.meta.env.VITE_ID}`);
    ...
    const editFormik = useFormik({
      ...
      onSubmit: async (values, { resetForm }) => {
        setIsLoadingOvalupdate(true);
          ...
          try {
              await axios.patch(
                  `${import.meta.env.VITE_URL_BACKEND}/siteData/${import.meta.env.VITE_ID}`,
                  siteData,
                  {
                      headers: {
                          "Authorization": token,
